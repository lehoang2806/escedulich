    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        if (response.data.length > 0) {
          ...
          const hostIds = [...new Set(response.data.map((service: ServiceResponse) => service.HostId))]
          console.log(`🔍 [useServices] Kiểm tra trạng thái của ${hostIds.length} Host(s)`)
            ...
            hostIds.map(async (hostId) => {
              try {
                ...
                if (userData.IS_BANNED === true || userData.IsActive === false) {
                  ...
                  console.log(`🚫 [useServices] Host ${hostId} bị khóa - ẩn dịch vụ`)
                }
              } catch (err) {
                // Nếu không lấy được thông tin Host, vẫn hiển thị dịch vụ
                console.warn(`⚠️ [useServices] Không thể kiểm tra trạng thái Host ${hostId}:`, err)
              }
            })
          )
          
          // Filter ra các Service của Host không bị khóa
          const filteredServices = response.data.filter(
            (service: ServiceResponse) => !bannedHostIds.has(service.HostId)
          )
          
          console.log(`✅ [useServices] Sau khi lọc Host bị khóa: ${filteredServices.length}/${response.data.length} service(s)`)
